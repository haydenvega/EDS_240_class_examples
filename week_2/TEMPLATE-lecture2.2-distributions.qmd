...
mko_clean %>% #month.name is a vector built into R for this purpose 
  mutate(month_name = factor(month_name, levels = month.name)) %>%  #factors are ordinal, character strings are not! if you want to order some characters, convert to a factor
...
mko_clean |> 
  filter(month_name %in% c("April", "June", "October")) |> 
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_density(alpha = 0.5, adjust = 0.5) + #adjust controls smoothing
...
ggplot(mko_clean, aes(Temp_bot, month_name, fill = after_stat(x)))+
...
mko_clean
mko_clean %>% 
  ggplot( aes(month_name, Temp_bot))+
  geom_boxplot() +
