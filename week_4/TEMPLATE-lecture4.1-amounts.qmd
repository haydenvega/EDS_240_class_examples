...
jobs_clean %>% 
  ...
  ggplot(aes(occupation, total_earnings)) + #flipx&y can flip here and switch the #order here in code or use coord_flip
  geom_col()+
...
test <- jobs_clean |> 
  ...
  mutate(sex = str_remove(string = group, pattern = "total_earnings"))+
...
ggplot(heatmap_order, aes(year, y = occupation, fill = total_earnings)) +
  geom_tile()+
