...
# ~ CODE ALONG HERE ~
  ...
  ggplot()+
  geom_point(data = penguins, aes(bill_length_mm, bill_depth_mm, color = species))+
  geom_point(data = penguins_summary, 
  mapping = aes(x = mean_bill_length_mm, y = mean_bill_depth_mm, fill = species), 
  size = 6, shape = 24, color = "darkorange")
```
...
ggplot(data = penguins,
       mapping = aes(x = bill_length_mm, y = bill_depth_mm,
                     color = species))+ #when you map it at the global level here, all subsequent layers apply to these options, which is why we have three lines of best fit
       geom_point() +
...
# ~ CODE ALONG HERE ~
ggplot(data = penguins,
       mapping = aes(x = bill_length_mm, y = bill_depth_mm))+ #when you map it at the global level here, all subsequent layers apply to these options, which is why we have three lines of best fit; to fix this, we put the color on a subsequent layer, not a global one so we don't get that grouping between three lines. 
...
#| warning: false
# ~ CODE ALONG HERE ~
ggplot(penguins, aes(species, fill = island ))+ #the fills are "stacked", it's what position does
